...
def read_yaml_spec(file):
    ...
    if file.endswith(".ipynb"):
        ...
        yaml_cell = [
            ...
            if cell["cell_type"] == "markdown" and cell["source"].startswith("```yaml")
        ][0]["source"]
...
def get_spec_external_dependencies(spec):
    ...
    external_dependencies["external_dependencies"] = (
        ...
        if "external_dependencies" in spec.keys()
        else None
    )
...
def get_external_dependencies(yaml_specs):
    ...
    external_dependencies = [
        ...
        if external_dependency["external_dependencies"] is not None
    ]
    return external_dependencies
...
def set_dependencies(yaml_specs, tasks, latest_only=True, **kwargs):
    dependencies = [
        (s["task_id"], get_yaml_spec_dependencies(s, tasks[s["task_id"]]))
        for s in yaml_specs
    ]
    ...
    if latest_only:
        latest_only_operator = LatestOnlyOperator(
            task_id="latest_only", dag=kwargs["dag"]
        )
    ...
    for task in external_dependencies:
        ...
        for external_dependency in external_dependencies:
            ...
            task_name = (
                "wait_for_DAG_" + external_dag
                if wait_for_whole_dag
                else "wait_for_" + external_task
            )
    ...
    for task_id, task_depends_on in dependencies:
        valid_dependencies = [
            d for d in task_depends_on if d in tasks.keys() and d != task_id
        ]

        if (
            len(valid_dependencies) == 0
            and latest_only
            and task_id not in tasks_with_external_dependencies
        ):
...
class GustyDAG(airflow.DAG):
    ...
    def __init__(self, directory, latest_only=True, **kwargs):
        ...
        latest_checked = (
            default_args["latest_only"]
            if "latest_only" in default_args.keys()
            else latest_only
        )
